...
Ray* generateRay(int i, int j, int eVar) {
	...
	VEC3F eyePosition(0, 0, -150);  // eye/camera position
...
int main () {
	...
	// small sphere
	spheres[0].center = VEC3F(0, 0, -10);
	spheres[0].color = VEC3F(1.0f, 0.0f, 0.0f);
	spheres[0].radius = .165;
			       
	// ground sphere
	spheres[1].center = VEC3F(0, -1e5, 0);
	spheres[1].color = VEC3F(1.0f, 1.0f, 1.0f);
	spheres[1].radius = 1e5;
	    
	// light1
	lights[0].origin = VEC3F(0, 30, -10);  // light location	
	lights[0].color = VEC3F(1.0f, 1.0f, 1.0f);
