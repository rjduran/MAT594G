...
void setup() {
  ...
  modifier.setDistance(20);// extrusion distance, set to 0 for inset faces
  ...
  twistModifier.setAngleFactor(0.02);

  for (int i=0; i< numcells; i++) {
    cells[i].modify(twistModifier);
  }
...
void draw() {
  ...
  L=new WB_Line(-10, 0, 0, -10+mouseX*0.5, 0, 10);

  twistModifier.setTwistAxis(L);// Twist axis
  //twistModifier.setAngleFactor(.004);
...
void createMesh() {
  ...
  multiCreator.setCutoff(40);// maximum radius of cell
