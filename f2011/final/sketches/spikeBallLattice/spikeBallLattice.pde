...
void setup() {
  ...
  latticeModifier=new HEM_Lattice();
  latticeModifier.setWidth(10);// desired width of struts
  latticeModifier.setDepth(10);// depth of struts
  latticeModifier.setThresholdAngle(1.5*HALF_PI);// treat edges sharper than this angle as hard edges
  latticeModifier.setFuse(true);// try to fuse planar adjacent planar faces created by the extrude
  latticeModifier.setFuseAngle(0.05*HALF_PI);// threshold angle to be considered coplanar
  mesh.modify(latticeModifier);
...
void draw() {
  ...
  //latticeModifier.setWidth(map(mouseX, 0, width, 1, 20));// desired width of struts
  latticeModifier.setDepth(map(mouseX, 0, width, 1, 20));// depth of struts  
  mesh.modify(latticeModifier);
}
