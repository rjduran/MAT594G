...
void setup() {
  ...
  container=new HE_Mesh(new HEC_Geodesic().setRadius(250).setLevel(3));
  ...
  for(int i=0;i<numpoints;i++) {
    points[i][0]=random(-d,d);
    points[i][1]=random(-d,d);
    points[i][2]=random(-d,d);
  }
  ...
  multiCreator.setN(40);//number of points, can be smaller than number of points in input. 
  multiCreator.setContainer(container);// cutoff mesh for the voronoi cells, complex meshes increase the generation time
  multiCreator.setOffset(4);// offset of the bisector cutting planes, sides of the voronoi cells will be separated by twice this distance
